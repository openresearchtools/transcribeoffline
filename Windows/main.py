...
# --------------------------- Performance helpers (NEW, no workers) ---------------------------
...
def _recommended_threads() -> tuple[int, int]:
    """
    Returns (threads, interop_threads) based on TRANSOFF_MODE.

    Modes:
      - auto (default): reserve 1 core if n>=4
      - max: use ALL logical cores

    (No 'workers' here; processing remains one stage at a time.)
    """
    n = _cpu_logical_cores()
    mode = (os.getenv("TRANSOFF_MODE") or "auto").strip().lower()

    if mode == "max":
        reserve = 0
    else:
        reserve = 1 if n >= 4 else 0

    threads = max(1, n - reserve)
...
def _apply_perf_env_caps():
    """Apply BLAS/OMP caps to current process so CPU math respects the mode."""
    try:
...
# Apply caps now (child processes will read TRANSOFF_MODE and re-derive)
...
LLAMA_VENDOR_DIR = VENDOR / "llama.cpp"
...
# Default Qwen3 Instruct GGUF folder+file (shipped with the app)
...
def ensure_ffmpeg_on_path() -> str:
    ff_dir = VENDOR / "ffmpeg"
    ff = ff_dir / ("ffmpeg.exe" if sys.platform.startswith("win") else "ffmpeg")
    if not ff.exists():
        alt = VENDOR / ("ffmpeg.exe" if sys.platform.startswith("win") else "ffmpeg")
        if alt.exists(): ff = alt
    if not ff.exists():
        messagebox.showerror("FFmpeg missing",
            f"Place ffmpeg.exe + DLLs under:\n{VENDOR}/ffmpeg")
        raise SystemExit(1)
    os.environ["PATH"] = f"{str(ff.parent)}{os.pathsep}{os.environ.get('PATH','')}"
    return str(ff)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(APP_NAME)
        self.geometry("1260x880"); self.minsize(1120,760)
        _apply_icon(self)
        ...
        # LLM state (defaults; params dialog can edit them)
        ...
        # Performance mode (NEW; single-stage-at-a-time philosophy)
        self.perf_mode_var = tk.StringVar(
            value=("Max power" if os.getenv("TRANSOFF_MODE","auto")=="max" else "Auto (balanced)")
        )
        self._apply_perf_mode_to_env(initial=True)
        ...
        # Log chosen perf settings (threads only; no workers here)
        try:
            t, io = _recommended_threads()
            self._post("log", f"[Perf] CPU cores={_cpu_logical_cores()} • threads={t} • torch interop={io} • mode={os.getenv('TRANSOFF_MODE','auto')}")
        except Exception:
            pass
    ...
    def _apply_perf_mode_to_env(self, initial: bool=False):
        label = self.perf_mode_var.get().strip().lower()
        mode = "max" if label.startswith("max") else "auto"
        os.environ["TRANSOFF_MODE"] = mode
        _apply_perf_env_caps()
        if not initial:
            t, io = _recommended_threads()
            self._post("log", f"[Perf] mode={mode} → threads={t}, interop={io}")
    ...
    def _build_layout(self):
        ...
        # Performance control (NEW)
        perf_frame = ttk.Frame(ctrl1); perf_frame.pack(side="right")
        ttk.Label(perf_frame, text="Performance:").pack(side="left", padx=(0,4))
        perf_cb = ttk.Combobox(perf_frame, width=24, state="readonly",
                               values=["Auto (balanced)", "Max power"],
                               textvariable=self.perf_mode_var)
        perf_cb.pack(side="left")
        perf_cb.bind("<<ComboboxSelected>>", lambda e: self._apply_perf_mode_to_env(initial=False))
        ...
        output_frame = ttk.Labelframe(root, text="Output"); output_frame.pack(fill="both", expand=True, **pad)

        # LLM + text size controls ABOVE the text
        top_tools = ttk.Frame(output_frame); top_tools.pack(fill="x", padx=4, pady=(4,4))

        # Text size
        ttk.Label(top_tools, text="Text size:").pack(side="left", padx=(0,6))
        ttk.Button(top_tools, text="A-", command=lambda: self._bump_text_size(-1)).pack(side="left")
        ttk.Button(top_tools, text="A+", command=lambda: self._bump_text_size(+1)).pack(side="left", padx=(4,0))
        ttk.Button(top_tools, text="Reset", command=lambda: self._set_text_size(10)).pack(side="left", padx=(4,10))

        # LLM quick actions + Model Parameters
        ttk.Label(top_tools, text="LLM Target:").pack(side="left")
        ttk.Combobox(top_tools, textvariable=self.llm_target_lang_var, width=18, state="readonly",
                     values=LLM_LANG_NAMES).pack(side="left", padx=(2,10))
        ttk.Button(top_tools, text="Model Parameters…", command=self._open_llm_params).pack(side="left", padx=(0,10))
        ttk.Button(top_tools, text="Translate (keep ts)", command=self._llm_translate_selected).pack(side="left", padx=4)
        ttk.Button(top_tools, text="Summarise", command=self._llm_summarise_selected).pack(side="left", padx=4)
        ttk.Button(top_tools, text="Correct (keep ts)", command=self._llm_correct_selected).pack(side="left", padx=4)
        ttk.Button(top_tools, text="Custom Prompt…", command=self._llm_custom_prompt_dialog).pack(side="left", padx=8)
        ...
        sp = ttk.Frame(output_frame); sp.pack(fill="x", padx=4, pady=(0,2))
        ttk.Label(sp, text="Speed ×").pack(side="left")
        ttk.Entry(sp, textvariable=self._play_speed_var, width=6).pack(side="left", padx=(4,10))
        ttk.Button(sp, text="Apply to next play",
    ...
    def _save_preview_to_edited(self):
        ...
        out_path = f"{base}.edited{ext}" if not base.endswith(".edited") else f"{base}{ext}"
        try:
    ...
    # LLM actions — real implementation is in PART 2/2
...
def _proc_transcribe_entry(job: dict, out_q):
    ...
    try:
        ...
        out_q.put(("log", f"[Transcribe] loading (mode={os.getenv('TRANSOFF_MODE','auto')}, threads={threads}, workers=1)"))
...
def _find_llama_binary():
    d = _llama_vendor_dir()
    for c in [d/"llama-cli.exe", d/"llama-cli", d/"main.exe", d/"main", d/"llama.exe",
              d/"bin/llama-cli.exe", d/"bin/llama-cli", d/"bin/main.exe", d/"bin/main", d/"bin/llama.exe"]:
...
class _LlamaOnceMinimal:
    ...
    def run_to_file(
        ...
        # add only supported sampling flags if provided
...
def _llm_run_over_outputs(self, task: str, custom_prompt: str | None = None):
    ...
    def _worker():
        ...
        threads, _ = _recommended_threads()
        self._post("log", f"[LLM] starting (mode={os.getenv('TRANSOFF_MODE','auto')}, threads={threads})")
        try:
            runner = _LlamaOnceMinimal()

            # read GUI params safely (same pattern as max_new)
            try:
